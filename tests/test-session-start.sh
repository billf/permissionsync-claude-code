#!/usr/bin/env bash
# test-permissionsync-session-start.sh — unit tests for permissionsync-session-start.sh
set -euo pipefail
...
# Helper: create a stub permissionsync-sync.sh in a tmp dir alongside permissionsync-session-start.sh
make_env() {
	local dir="$1"
	mkdir -p "$dir"
	cp "${SCRIPT_DIR}/../permissionsync-session-start.sh" "$dir/"
}

run_hook() {
	local dir="$1"
	bash "${dir}/permissionsync-session-start.sh" </dev/null
}

# --- Test 1: Silent when permissionsync-sync.sh is absent ---
...
assert_eq "no output when permissionsync-sync.sh is absent" "" "$output"

# --- Test 2: Silent when no pending rules ---
NOCHANGE_DIR="${TMP_DIR}/nochange"
make_env "$NOCHANGE_DIR"
cat >"${NOCHANGE_DIR}/permissionsync-sync.sh" <<'STUB'
#!/usr/bin/env bash
echo "2c2"
echo "< old"
echo "---"
...
exit 0
STUB
chmod +x "${NOCHANGE_DIR}/permissionsync-sync.sh"
set +e
...
cat >"${PENDING_DIR}/permissionsync-sync.sh" <<'STUB'
#!/usr/bin/env bash
echo '> "Bash(git *)"'
echo '> "Bash(cargo *)"'
exit 0
STUB
chmod +x "${PENDING_DIR}/permissionsync-sync.sh"
output=$(run_hook "$PENDING_DIR" 2>&1)
assert_contains "shows new rule count" "2 new rule(s)" "$output"
assert_contains "shows --apply command" "permissionsync-sync.sh --apply" "$output"
assert_contains "shows --refine --apply command" "permissionsync-sync.sh --refine --apply" "$output"
...
# --- Test 5: Exits 0 when permissionsync-sync.sh fails ---
...
set +e
bash "${FAIL_DIR}/permissionsync-session-start.sh" </dev/null
exit_code=$?
set -e
